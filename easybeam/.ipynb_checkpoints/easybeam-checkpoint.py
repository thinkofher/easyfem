...
class Beam:
    ...
    def boundary(self, vertical_1, rotation_1, vertical_2, rotation_2):
        ...
        self.boundaries = [ self.vertical_1,
                            self.rotation_1,
                            self.vertical_2,
                            self.rotation_2]
    ...
    def loads(self, force_1=0, force_2=0, moment_1=0, moment_2=0, linear_load=0):
        try:
            ...
            self.system_loads = np.array([self.force_1, self.moment_1, self.force_2, self.moment_2])
        except:
            raise No_Data

    def internal_forces(self, solved_forces):
        self.__solved_forces__ = solved_forces
        self.internal_forces_array =  (-1)*np.transpose(np.matrix(self.system_loads)) + self.stifness_matrix * np.transpose(np.matrix(self.__solved_forces__))
        self.internal_forces_array = np.array( np.transpose( self.internal_forces_array ) )[0]
...
class BeamSolver:
    ...
    def internal_agregation(self):
        ...
        for i in range(len(matrixes)):
            ...
            self.global_stiffness_matrix = self.global_stiffness_matrix + current_matrix
    ...
    def internal_boundaries(self):
        ...
        for i in range(len(vectors)):
            ...
            self.global_boundary_vector = self.global_boundary_vector + current_vector
        
        self.global_boundary_vector = np.array(list(map(bool,self.global_boundary_vector)))
    ...
    def internal_system_loads(self):
        ...
        for i in range(len(vectors)):
            current_vector = np.zeros([self.n])
            current_vector[(0+2*i):(4+2*i)] = vectors[i]
            self.global_system_loads_vector = self.global_system_loads_vector + current_vector
    ...
    def apply_boundaries(self): 
        ...
        for i in range(np.size(self.global_boundary_vector,0)):
            if self.global_boundary_vector[i]:
                self.boundariezed_stiffness_matrix = np.delete(self.boundariezed_stiffness_matrix, delete_counter, 0)
                self.boundariezed_stiffness_matrix = np.delete(self.boundariezed_stiffness_matrix, delete_counter, 1)
                self.boundariezed_system_loads_vector = np.delete(self.boundariezed_system_loads_vector, delete_counter) 
            else:
                delete_counter += 1
                self.__solved_vector__[i] = True

    def solver(self):
        self.unknowns_solved = lp.solve(self.boundariezed_stiffness_matrix, self.boundariezed_system_loads_vector)
        self.global_solvings_vector = np.zeros([self.n])

        unknowns_counter = 0
        for i in range(len(self.__solved_vector__)):
            if self.__solved_vector__[i]:
                self.global_solvings_vector[i] = self.unknowns_solved[unknowns_counter]
                unknowns_counter += 1
        
        for counter,beam_element in enumerate(self.beams):
            beam_element.internal_forces(self.global_solvings_vector[(0+2*counter):(4+2*counter)])
...
def discretization(beam, number_of_elements):
    ...
    for i in range(number_of_elements):
        ...
        # o klasie Beam
        current_beam = Beam(length_of_element, beam.youngs_modulus, beam.section)

        # okreslam warunki brzegowe poszczegolnych elementow
        # i nadaje obciazenia
        if i == 0:
            current_beam.boundary(beam.vertical_1, beam.rotation_1, False, False)
            current_beam.loads(force_1=beam.init_force_1,moment_1=beam.init_moment_1,linear_load=beam.linear_load)
        elif i == (number_of_elements-1):
            current_beam.boundary(False, False, beam.vertical_2, beam.rotation_2)
            current_beam.loads(force_2=beam.init_force_2,moment_2=beam.init_moment_2,linear_load=beam.linear_load)
        else:
...
def momments_array(*beams):
    ...
    moments_array = []
    for beam in beams:
        moments = [ beam.internal_forces_array[1], -1*beam.internal_forces_array[3] ]
...
def shears_array(*beams):
    ...
    shears_array = []
    for beam in beams:
        shears = [ beam.internal_forces_array[0], -1*beam.internal_forces_array[2] ]
        for shear in shears:
